...
dec <- modelGeneVar(sce.filtered.nonartifact)
hvgs <- getTopHVGs(dec, n = 2000)
sce.filtered.nonartifact <- runPCA(sce.filtered.nonartifact, subset_row = hvgs)
g <- buildSNNGraph(sce.filtered.nonartifact, use.dimred = "PCA")
clusters <- igraph::cluster_louvain(g)$membership
colLabels(sce.filtered.nonartifact) <- factor(clusters)
sce.filtered.nonartifact <- runUMAP(sce.filtered.nonartifact, dimred = "PCA")
```
